...
(function ($) {
    $.fn.EasyTree = function (options) {
        var defaults = {
            ...
            addable: true,
            i18n: {
                deleteNull: 'Select a category to delete',
                deleteConfirmation: 'Delete this category?',
                confirmButtonLabel: 'Okay',
                editNull: 'Select a category to edit',
                editMultiple: 'Only one category can be edited at one time',
                addMultiple: 'Select a category to add a new category',
        ...
        var warningAlert = $('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><strong></strong><span class="alert-content"></span> </div> ');
        var dangerAlert = $('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><strong></strong><span class="alert-content"></span> </div> ');

        var createInput = $('<div class="input-group"><input type="text" class="form-control"><span class="input-group-btn"><button type="button" class="btn-sm btn-primary confirm"></button></span><span class="input-group-btn"><button type="button" class="btn-sm btn-primary cancel"></button> </span> </div> ');
        ...
        this.each(function () {
            ...
            $.each($(easyTree).find('ul > li'), function() {
                ...
                if($(this).is('li:has(ul)')) {
                    ...
                    $(children).remove();
                    //angie text = $(this).text();
					text = $(this).clone().children().remove().end().text(); //angie
					
					catcode = $("div .catcode:eq("+count+")").text();
					removecode =$("div .removecode:eq("+count+")").text();
					//console.log('count',testing);
					//catcode = $('.catcode').first().text(); //angie
					
					$(this).html('<span><span class="glyphicon"></span><a href="javascript: void(0);"></a> </span> <a catcode="'+catcode+'">'+catcode+'</a>'); //angie
					...
					text = $(this).clone().children().remove().end().text(); //angie
					catcode = $('.catcode').first().text(); //angie
					$(this).html('<span><span class="glyphicon"></span><a href="javascript: void(0);"></a> </span> <a catcode="'+catcode+'">'+catcode+'</a>'); 
            ...
            // addable
            if (options.addable) {
                $(easyTree).find('.easy-tree-toolbar').append('<div class="create"><button class="btn btn-sm btn-primary"><span class="glyphicon glyphicon-plus"></span></button></div> ');
            ...
            // deletable
            if (options.deletable) {
                $(easyTree).find('.easy-tree-toolbar').append('<div class="remove"><button class="btn btn-sm btn-primary disabled"><span class="glyphicon glyphicon-remove"></span></button></div> ');
                $(easyTree).find('.easy-tree-toolbar .remove > button').attr('title', options.i18n.deleteTip).click(function () {
                    ...
                    if (selected.length <= 0) {
                        ...
                        $(easyTree).find('.alert .alert-content').html(options.i18n.deleteConfirmation)
                            .append('<a style="margin-left: 10px;" class="btn-sm btn-primary confirm"></a>')
