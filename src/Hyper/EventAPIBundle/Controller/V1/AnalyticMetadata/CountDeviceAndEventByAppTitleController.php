...
class CountDeviceAndEventByAppTitleController extends APIBaseController
{
	...
	protected function parseToResult($result)
	{
		...
		foreach ($result as $appTitleId => $listAppTitle) {
			foreach ($listAppTitle as $appTitle) {
				$ret[$appTitle['client_name']][$appTitle['app_title']][$appTitle['platform']][$appTitle['app_id']] =array(
						'app_title_id' => $appTitle['app_title_id'],
						'device_count' => $appTitle['device_count'],
						'event_count' => $appTitle['event_count'],
						'ghost_user_count' => $appTitle['ghost_user_count'],
						'dormant_user_count' => $appTitle['dormant_user_count'],
					);
			}
		}
